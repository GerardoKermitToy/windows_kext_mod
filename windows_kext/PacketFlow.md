...
The cache stores TCP and UDP keys, direction, process ID, verdict, redirect state, and activity timestamps. Native WFP flow deletion, endpoint closure, and resource release mark tracked connections as ended and emit their lifecycle event. Cleanup removes ended entries after their one-minute late-packet grace period. A ten-minute inactivity watchdog emits END for every live UDP entry, including one whose native WFP cleanup is delayed; it removes only Portmaster's bookkeeping and does not abort the WFP flow or close the application's socket. Other stale untracked cache state is discarded as before.
